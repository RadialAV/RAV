...
function getFeaturedRelease() {
    try {
        ...
        const pool = releases.filter(r => !r.comingSoon);
        const list = pool.length ? pool : releases;
...
function renderHomeFeatured() {
    ...
    if (!el) return;
    const rel = getFeaturedRelease();
    const artists = (typeof getAllArtists === 'function') ? getAllArtists() : [];
    // Encontrar artista com mais faixas
    let bestArtist = '';
    ...
    const hasRel = !!rel;
    const bgStyle = hasRel && rel.coverUrl ? `background-image:url('${rel.coverUrl}')` : '';
    const year = hasRel && rel.year ? ` • ${rel.year}` : '';
    const subtitle = hasRel && rel.subtitle ? `<div class="feat-sub">${rel.subtitle}</div>` : '';
    const ids = hasRel ? JSON.stringify(rel.trackIds || []) : '[]';
    const relTile = hasRel ? `
    ...
    const artistTile = bestArtist ? `
      <div class="featured-card">
        <div class="featured-overlay"></div>
        <div class="featured-content">
          <div class="feat-eyebrow">Artista</div>
          <div class="feat-title">${bestArtist}</div>
          <div class="feat-meta">Explorar músicas</div>
          <div class="feat-actions">
            <button class="btn btn-primary btn-sm" onclick='(window.showArtistDetails ? showArtistDetails(${JSON.stringify(bestArtist)}) : 0)'>Abrir</button>
          </div>
        </div>
      </div>` : '';
