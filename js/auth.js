    ...
    try {
      ...
      if (data && data.url) {
        ...
        if (!w) { window.location.href = data.url; }
          ...
          // Fallback: se o utilizador fechar o popup sem autenticar, reativar botão
          const chk = setInterval(() => {
            try {
              if (!window.__authPopup || window.__authPopup.closed) {
                clearInterval(chk);
  ...
  window.addEventListener('message', (e) => {
    try {
      if (e && e.data && e.data.type === 'supabase:auth') {
        updateAuthUI();
        try { if (window.__authPopup && !window.__authPopup.closed) window.__authPopup.close(); } catch(_) {}
        try { signinBtn.disabled = false; signinBtn.textContent = signinBtn.dataset.prevText || 'Entrar com Google'; } catch(_) {}
      }
    } catch(_) {}
  });
