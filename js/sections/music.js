...
function createLabelCardSimple(meta){
  const name = meta && meta.name ? meta.name : 'Label';
  const logo = (meta && meta.logo_url) ? `<img src="${meta.logo_url}" alt="${name}" class="release-cover-img">` : (name.charAt(0).toUpperCase());
  return `
    <div class="release-card" onclick="(window.showLabelDetails ? showLabelDetails('${meta.id}') : 0)">
