...
export async function fetchCatalogFromSupabase() {
  ...
  const [trRes, relRes, vidRes] = await Promise.all([
    ...
    supa.from('releases').select('id,title,subtitle,year,cover_url,total_duration,description,genres,credits,featured,coming_soon'),
  ...
  const tracks = sortedTracks.map((t, idx) => {
    const localId = idx + 1;
  ...
  // Fallbacks de capas: completar capas em releases/tracks quando faltam
  try {
