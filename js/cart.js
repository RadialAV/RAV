...
export function confirmCheckout(){
  try { if (window.showToast) window.showToast('Compra confirmada (placeholder). Integração de pagamentos em breve.', 'success'); } catch(_){ }
  try { dataClearCart(); } catch(_){}
  updateCartBadge();
  if (window.closeModal) window.closeModal('checkoutModal'); else { const m = qs('checkoutModal'); if (m) m.style.display='none'; }
}
